...
binary_classifier = tfk.models.load_model(
    '/home/simones95/EnvCrime/Local20_PS32_K3.h5',
    custom_objects={'RandomNoiseLayer': RandomNoiseLayer,
                    ...
                    'MILInputLayer':MILInputLayer,
                    'FeaturesMapsLayer':FeaturesMapsLayer,
                    'AggregateFeaturesLayer':AggregateFeaturesLayer})

patch_weight_model = tfk.Model(inputs=binary_classifier.get_layer('mil_input').output,
                      outputs=[binary_classifier.get_layer('attentionWeights').output, binary_classifier.get_layer('localGaps').output] )

patch_probability_model = tfk.Model(inputs=binary_classifier.get_layer('MIL_features').output,
...
for file_name in os.listdir('/dataset/new_simones/AW2_segmentation/'):
    ...
    image = tf.image.central_crop(image,1024/max_dim)
    segmentation = tf.image.central_crop(segmentation,1024/max_dim).numpy()
    max_dim = 1024
...
for file_name in os.listdir('/dataset/new_simones/AW2_segmentation/'):
    ...
    image = tf.image.central_crop(image,1024/1088)
    segmentation = tf.image.central_crop(segmentation,1024/1088).numpy()
    ...
    sp = np.zeros((1024, 1024, 1))
    wp = np.zeros((1024, 1024, 1))
    swp = np.zeros((1024, 1024, 1))
    w = np.zeros((1024, 1024, 1))
    count_patch = 0
    for i in range(0, 1024-patch_size+1, shift):
        for j in range(0, 1024-patch_size + 1, shift):
            sp[i:i + patch_size, j:j + patch_size, :] += np.ones((patch_size, patch_size, 1)) * local_probability[count_patch, 0]
            wp[i:i + patch_size, j:j + patch_size, :] += np.ones((patch_size, patch_size, 1)) * weights[count_patch, 0]
            w[i:i + patch_size, j:j + patch_size, :] += np.ones((patch_size, patch_size, 1))
            count_patch += 1
